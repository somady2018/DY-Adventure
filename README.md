...
| id | 이름 | 타입 | 보상 |
|---|---|---|---|
| `base_wash` | 반짝반짝 정비 미션 | required | 생활력 +4 · 마음력 +1 |
| `base_read_15` | 지식의 문 열기 | choice | 지식 +4 · 탐구력 +2 |
| `base_school_bag` | 내일의 장비 챙기기 | required | 생활력 +3 · 탐구력 +3 |
| `base_meal` | 에너지 충전 작전 | required | 끈기 +3 · 용기 +2 |
| `base_room_clean` | 나만의 기지 정비 | choice | 생활력 +5 |
| `base_thank_you` | 마음빛 발견하기 | choice | 마음력 +5 · 협동력 +3 |

참고: 예전에 있던 "모험식 이름(storyTitle)" 필드는 화면 어디에도 쓰이지 않아
v0.8에서 제거했습니다. 템플릿 이름은 "퀘스트 이름" 하나만 사용합니다.
