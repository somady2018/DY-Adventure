...
`schemaVersion: 8`(이전 데이터와 호환, 로딩 시 자동 마이그레이션)이고,
...
- 예전 데이터는 앱 로딩 시 `schemaVersion: 8`로 정규화됩니다.
...
(스키마 8부터 기존 저장 데이터에도 자동 복구되어 실제로 분할 지급됩니다.
템플릿 관리의 "보조 보상"으로 같은 형태의 템플릿을 직접 만들 수도 있습니다.)
